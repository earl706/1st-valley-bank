...
export default function PropertiesForSaleVehicles() {
	...
	const fetchVehicles = async (page = 1) => {
		...
		try {
			...
			const response = await propertyService.getVehicles(params);
			if (response.success) {
	...
	if (loading && vehicles.length === 0 && currentPage === 1) {
		return (
						...
						<div className="mb-6 rounded-lg border border-white/20 bg-white p-4 shadow-sm">
							<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
								<div>
									...
									<select
										...
										className="w-full rounded-lg border border-[#396131]/30 bg-white px-3 py-2 text-sm text-[#396131] focus:border-[#396131] focus:ring-2 focus:ring-[#396131]/20 focus:outline-none"
									>
							...
							<div className="mt-4 flex items-center justify-end gap-3">
								<button
									...
									className="rounded-lg border border-[#396131]/30 bg-white px-4 py-2 text-sm font-medium text-[#396131] transition hover:bg-[#396131]/5"
								>
									Reset
								</button>
								<button
